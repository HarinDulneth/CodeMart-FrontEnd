...
export function EditProject({ project, onProjectUpdate }: EditProjectProps) {
  const [currentProject, setCurrentUserState] = useState<User>(user);
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      let profilePictureUrl = currentProject.profilePicture;
      let userData = {};
      ...
      const response = await api.users.update(currentProject.id, userData);
  ...
  const uploadProfilePicture = async (): Promise<string | null> => {
    ...
    const fileName = `profile-${currentProject.id}-${Date.now()}.${fileExt}`;
